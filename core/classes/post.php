...
public function profileTweet($user_id, $profileId, $num){
...
<?php
    ...
    public function postCount($profileId){
        $stmt= $this->pdo->prepare("SELECT * FROM users LEFT JOIN profile ON users.user_id = profile.userId LEFT JOIN post ON post.userId = users.user_id WHERE post.userId = :user_id ORDER BY post.postedOn DESC ");
    ...
    public function postImageUpd($user_id, $post_id, $editText, $postImage){
        $stmt= $this->pdo->prepare('UPDATE post SET post = :editText, postImage = :postImage WHERE post_id =:post_id AND userId = :user_id');
        $stmt->bindParam(":post_id", $post_id, PDO::PARAM_INT);
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
        $stmt->bindParam(":editText", $editText, PDO::PARAM_INT);
    ...
    public function commentFetch($postid){
        $stmt = $this->pdo->prepare("SELECT * FROM comments INNER JOIN profile ON comments.commentBy = profile.userId LEFT JOIN users ON users.user_id = profile.userId WHERE comments.commentOn = :postid LIMIT 10");
        $stmt->bindParam(":postid", $postid, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
...
public function commentUpd($userid, $postid, $editedTextVal, $commentid){
        $stmt = $this->pdo->prepare("UPDATE comments SET comment = :editedText WHERE commentID =:commentid AND commentBy = :userid");
        $stmt->bindParam(":commentid", $commentid, PDO::PARAM_INT);
        $stmt->bindParam(":userid", $userid, PDO::PARAM_INT);
   ...
   public function totalShareCount($postid){
    ...
    public function searchText($search){
        $stmt = $this->pdo->prepare("SELECT * FROM users LEFT JOIN profile ON users.user_id = profile.userId WHERE  users.username LIKE ? OR users.full_name LIKE ?");
  ...
  public function requestCheck($userid, $profileId){
    ...
    public function followCheck($profileId, $userid){
     ...
     public function followerCount($profileId){
        $stmt = $this->pdo->prepare("SELECT * FROM follow WHERE  receiver = :profileid");
    ...
    public function followingCount($profileId){
        $stmt = $this->pdo->prepare("SELECT * FROM follow WHERE sender = :profileid");
...
$userdata = $this->userdata($profileid);
   ...
   public function lastPersonMsg($userid){
     ...
     public function notification($userid){
        $stmt = $this->pdo->prepare("SELECT * FROM notification LEFT JOIN profile ON notification.notificationFrom = profile.userId LEFT JOIN users ON profile.userId = users.user_id WHERE notification.notificationFor = :userid AND notification.type != 'message' ORDER BY notification.notificationOn DESC; ");
 ...
 public function msgNotification($userid){
    ...
    public function notificationCount($userid){
        $stmt = $this->pdo->prepare("SELECT * FROM notification LEFT JOIN profile ON notification.notificationFrom = profile.userId LEFT JOIN users ON profile.userId = users.user_id WHERE notification.notificationFor = :userid AND notification.notificationCount = '0'  AND notification.type != 'message' ORDER BY notification.notificationOn DESC; ");
        $stmt->bindValue(':userid', $userid, PDO::PARAM_INT);
         $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    public function notificationCountReset($userid){
        $stmt = $this->pdo->prepare("UPDATE notification SET notificationCount = '1' WHERE notificationFor = :userid AND type != 'message' AND  notificationCount = '0' ");
        $stmt->bindValue(':userid', $userid, PDO::PARAM_INT);
         $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
  ...
  public function notificationStatusUpdate($userid, $notificationId){
    ...
    public function postScheduleDetails($postid){
        $stmt = $this->pdo->prepare("SELECT * FROM users LEFT JOIN profile ON users.user_id = profile.userId LEFT JOIN postSchedule ON postSchedule.userId = users.user_id WHERE postSchedule.post_id = :postid  ");
        $stmt->bindValue(':postid', $postid, PDO::PARAM_INT);
         $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ);
    }
 ...
 public function allUsersFilter(){
    ...
    public function userData($fcode){
     ...
     public function sharedVidLink($vidCode){
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM post WHERE vid = :vidCode");
    ...
    public function postScheduleList($userid){
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM postSchedule LEFT JOIN profile ON postSchedule.postBy = profile.userId LEFT JOIN users ON profile.userId = users.user_id  WHERE postSchedule.postBy = :userid ");
        $stmt->bindParam(":userid", $userid, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
    public function getOnlySchedule(){
        ...
        $stmt = $this->pdo->prepare("SELECT * FROM postSchedule");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
}
