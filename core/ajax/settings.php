...
if(isset($_POST['changeName'])){
    ...
    $loadFromUser->update('profile', $userid, $fields = array('full_name'=>$firstName));
    $loadFromUser->userUpdate('users', $userid, $fields = array('full_name'=>$firstName));
...
if(isset($_POST['userLink'])){
    ...
    $userid = $_POST['userid'];
$error = '';
    if(DB::query('SELECT username FROM users WHERE username = :userLink', array(':userLink'=>$userLink))){
        $error = 'Username is already in use.';
    }else{
        $loadFromUser->userUpdate('users', $userid, $fields = array('username'=>$userLink));
    }


   if($error == ''){ echo '<h3 style="color:#4caf50;">Username has changed successfully.</h3>' ; }else{echo $error; }
