...
public function checkInput($variable){
    ...
    public function checkEmail($email_mobile){
      ...
      public function adminCheckEmail($email_mobile){
        $stmt = $this->pdo->prepare("SELECT adminEmail FROM admin WHERE adminEmail = :email");
        $stmt->bindParam(":email", $email_mobile, PDO::PARAM_STR);
        $stmt->execute();
        $count = $stmt->rowCount();
        if($count > 0){
...
$sql = "INSERT INTO {$table}({$columns})VALUES ({$values})";
    ...
    public function userData($profileId){
       $stmt = $this->pdo->prepare("SELECT * FROM users LEFT JOIN profile ON users.user_id = profile.userId WHERE users.user_id = :user_id");
        $stmt->bindParam(':user_id', $profileId, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_OBJ);
    }
public function usernameFetch($userid){
       $stmt = $this->pdo->prepare("SELECT username FROM users WHERE user_id = :userid");
...
public function timeAgoForCom($datetime){
    ...
    public function joinDate($datetime){
        $time = strtotime($datetime);
        $current = time();
        $seconds = $current-$time;
        $minutes = round($seconds/60);
...
public function deleteR($table, $array){
        $sql = "DELETE FROM `{$table}`";
        $where = " WHERE ";
        foreach($array as $name=>$value){
            $sql .= "{$where} `{$name}` = :{$name}";
